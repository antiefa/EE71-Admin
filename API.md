...
### GetNetworkSettings / SetNetworkSettings — из прошивки
...
**Веб-интерфейс EE71 урезает список до `[0, 3]`** — режимы 2G и 3G доступны только через API.
...
Параметры `SendUSSD` не подтверждены: ни одна сборка его не вызывает. По именам полей рядом в `core_app` ожидается `{UssdContent, UssdType}`. Слова статьи «Скрытые функции прошивки» о том, что страница `ussd` отвечает «Не поддерживается», относятся к странице интерфейса, а не к методу.
